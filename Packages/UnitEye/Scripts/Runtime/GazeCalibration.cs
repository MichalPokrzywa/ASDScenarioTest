...
public class GazeCalibration : MonoBehaviour
{
    ...
    public string GUIMessage { get => _guiMessage; }
    ...
    public Image calibrationDotImage;
    ...
    public void StartCalibration()
    {
        ...
        //ResetPoints(0);
        if (calibrationDotImage != null && calibrationDot != null)
        {
            calibrationDotImage.sprite = Sprite.Create(
                calibrationDot,
                new Rect(0, 0, calibrationDot.width, calibrationDot.height),
    ...
    void LateUpdate()
    {
        ...
        if (_finishedRound)
        {
            ...
            currentRound++;
            int totalRounds = _presets.Count * maxRoundsPerPreset;
            Debug.Log($"Finished round {currentRound}/{totalRounds}, moving to pattern {_currentPreset + 1}");
            _currentPreset = (currentRound / maxRoundsPerPreset) % _presets.Count;
            ResetPoints(_currentPreset);
            _started = false;
            _showMessage = false;
            _guiMessage = $"Click to start next round\nPattern {_currentPreset + 1}/{_presets.Count} - Round {currentRound}/{totalRounds}";
            _isYielding = false;
            DrawPath(points);
        }

        //If done with all rounds or if we want to stop early, finish calibration
        int totalRoundsNeeded = _presets.Count * maxRoundsPerPreset;
        if (currentRound >= totalRoundsNeeded || _earlyStop)
        {
            _showMessage = true;
            _finished = true;

            Debug.Log($"Calibration finished! Collected {_xData.Count} data points");
    ...
    System.Collections.IEnumerator Training()
    {
        ...
        //Prepare GUI message
        var message = $"Calibration done!\nScreen size: {Functions.PixelsToMm(Screen.width) * 0.1f}x{Functions.PixelsToMm(Screen.height) * 0.1f}cm.\n";
        ...
        if (returnAfter)
            message += $"Click button to return.";
    ...
    private void UpdateCalibrationDot()
    {
        if (calibrationDotImage != null)
        {
            // Convert screen coordinates to canvas anchored position
            calibrationDotImage.rectTransform.anchoredPosition = new Vector2(
                _crossHairPos.x  - Screen.width / 2f,
                Screen.height / 2f -_crossHairPos.y 
            );

            calibrationDotImage.gameObject.SetActive(calibrationDot != null);
        }
    }

...
namespace UnitEye
{
    public class CSVData
    {
        ...
        public CSVData(float gazePixelX, float gazePixelY, float gazeNormalizedX, float gazeNormalizedY, float gazeNormalizedUnfilteredX, float gazeNormalizedUnfilteredY, float distanceToCamera, float eyeAspectRatio, bool blinking, DateTime timestampDay, float timestamp, List<string> AOIList)
        {
        ...
        public string SerializeCSVData()
        {
            ...
            bobTheStringBuilder.Append($";{TimeSpan.FromSeconds(timestamp).ToString(@"mm\:ss\.ff")}");
            //Add timestamp with milliseconds at the end
            bobTheStringBuilder.Append($";{timestampDay}:{timestampDay.Millisecond}");

            //Add unix timestamp in milliseconds
            bobTheStringBuilder.Append($";{((DateTimeOffset)timestampDay).ToUnixTimeMilliseconds()}");
            ...
            if (AOIList.Count > 0)
            {
                ...
                bobTheStringBuilder.Append($";{AOIList[0]}");
            }
            else
            {

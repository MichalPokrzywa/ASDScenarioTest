...
public class Gaze : MonoBehaviour
{
    ...
    public ASDAnalistManager ASDAnalistManager { get => asdAnalist; }
    public GazeCalibration CalibrationScript
    {
        get => _calibrationScript;
    }
    ...
    [SerializeField] ASDAnalistManager asdAnalist;
    ...
    public virtual void LateUpdate()
    {
        ...
        if (!PauseCSVLogging && csvLogger != null && csvLogger.isActiveAndEnabled)
        {
            ...
            csvLogger.Append(new CSVData(gazeLocation.x, gazeLocation.y, gazeLocation.x / Screen.width, gazeLocation.y / Screen.height, unfilteredGaze.x / Screen.width, unfilteredGaze.y / Screen.height, _distance, _eyeHelper.EyeFeature(), _blinking, now, timeRunning, aoiNameList));
            asdAnalist.AddEyeTrackingData(unfilteredGaze.x / Screen.width, unfilteredGaze.y / Screen.height,gazeLocation.x / Screen.width, gazeLocation.y / Screen.height, _distance, _eyeHelper.EyeFeature(), _blinking, aoiNameList[0]);
        }
        else
        {
            timeRunning = 0.0f;
    ...
    public void LoadCalibration(float speed = 9.0f, float padding = 20.0f, int rounds = 2)
    {
        ...
        _calibrationScript = gameObject.GetComponent<GazeCalibration>();
        _calibrationScript.StartCalibration();
        ...
        _calibrations = Calibrations.MLCalibration;

        //Calibration settings
        _calibrationScript.quitAfterCalibration = false;
        _calibrationScript.returnAfter = false;
